...
static PyObject* process_task(PyObject* self, PyObject* args) {
    ...
    */
    ...
    char* signature;
    char* on_failure;
    int priority, num_nodes, reduce, chunk_size, replicated, distributed, has_target, num_returns, time_out, rank;
    PyObject *values;
    PyObject *names;
    ...
    if(!PyArg_ParseTuple(args, "lssiiiiiiiiiiOOOOOOOOO", &app_id, &signature, &on_failure, &time_out, &priority,
                         &num_nodes, &reduce, &chunk_size, &replicated, &distributed, &has_target, &num_returns, &rank, &values, &names, &compss_types,
