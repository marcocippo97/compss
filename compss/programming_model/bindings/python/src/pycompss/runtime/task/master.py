        ...
        if CORE_ELEMENT_KEY in kwargs:
            ...
            for key, value in constraints.items():
                ...
                if (
                    ...
                    constraints[key] = self.user_function.__globals__[value]
                    self.constraint_args[key].set_is_static(False)
                else:
                    try:
                        args_dict = {
            ...
            for key, value in constraints.items():
                if not self.constraint_args[key].get_is_static():
                    ...
                    if param_name in kwargs:
                        ...
                        constraints[key] = self.user_function.__globals__[str(param_name)]
                    else:
                        try:
                            args_dict = {
                                self.param_args[i]: args[i]
    ...
    def inspect_task_arguments(self, args: tuple, kwargs: dict) -> None:
        ...
        for key in self.decorator_arguments.get_keys():
            ...
            if (
                ...
                if __debug__:
                    logger.debug("Detected dynamic task argument passed as a global variable")
                new_task_arguments[key] = self.user_function.__globals__[value]
            else:
                try:
                    args_dict = {
                        self.param_args[i]: args[i]
